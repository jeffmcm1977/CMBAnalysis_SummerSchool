...
def _deltaTOverTcmbToJyPerSr(freq_GHz, T0=2.7255):
    ...
    x = h * nu / (kB * T0)
    cNu = (
        2 * (kB * T0) ** 3 / (h**2 * c**2) * x**4 / (4 * (np.sinh(x / 2.0)) ** 2)
    )
    cNu *= 1e23
    return cNu
...
def greyBody(
    freq_GHz, beta=1.4, T_d=13.6
):  # Gispert:1.4, 13.6  https://arxiv.org/abs/astro-ph/0005554
    freq = freq_GHz * 1e9
...
def galaticDust_SED(freq_GHz, beta_d=1.48, t_d=19.6, freq_GHz_0=150, **kwargs):
    ...
    if "in_uk" in kwargs and kwargs["in_uk"] == True:
        return (
            greyBody(freq_GHz, beta=beta_d, T_d=t_d)
            / _deltaTOverTcmbToJyPerSr(freq_GHz)
            * 2.7255e6
        )
    ...
    **kwargs
    ...
    if mapType1 == "E" and mapType2 == "E":
        cls_tmp = (
            ...
            * scalePol
            * (ls / 100.0) ** n_d_e
            / galaticDust_SED(None, in_uk=True, **kwargs) ** 2
            / 100**2
            * 2
            * np.pi
        )
    ...
    **kwargs
    ...
    if mapType1 == "E" and mapType2 == "E":
        cls_tmp = (
            ...
            * scalePol
            * (ls / 100.0) ** n_s_e
            / synchrotron_SED(None, in_uk=True, **kwargs) ** 2
            / 100**2
            * 2
            * np.pi
        )  # /(2*np.pi)
...
def syncxdust_Cls(
    ...
    mapType2,
    correlationCoef=-0.1,
    synchrotron_fnc=synchrotron_Cl,
    galaticDust_fnc=galaticDust_Cl,
    lmax=8000,
    **kwargs
):
    cl11 = np.abs(synchrotron_fnc(mapType1, mapType1, lmax, **kwargs)) ** 0.5
    cl22 = np.abs(galaticDust_fnc(mapType2, mapType2, lmax, **kwargs)) ** 0.5
    return correlationCoef * cl11 * cl22


def gauss_beam(ell, fwhm_arcmin):
    tht_fwhm = np.deg2rad(fwhm_arcmin / 60.0)
...
class simple_sky_model:
    def __init__(
        self,
        camb_file="./CMB_fiducial_totalCls.dat",
        seed=1,
        pixRes_arcmin=4,
        lmax_sim=500,
    ):
        ...
        self.pixRes_arcmin = pixRes_arcmin / 180.0 / 60 * np.pi
        ...
        galShape /= np.mean(galShape**2) ** 0.5

        np.random.seed(seed)
        self.shape = shape
        self.wcs = wcs

        self.alm_cmb_T = curvedsky.rand_alm(
            cl_tt, lmax=lmax_sim
        )  # hp.synalm(cl_ee,lmax=3*nside-1,new=True)
    ...
    def observe(self, freq_GHz, noise_ukarcmin=3.0, beam_fwhm_arcmin=8.0):
        # np.random.seed(213114124+int(freq_GHz))

        beam = gauss_beam(
            np.arange(self.lmax_sim + 10), beam_fwhm_arcmin
        )  # hp.gauss_beam(beam_fwhm_arcmin*(np.pi/60./180),lmax=3*self.nside)
        beam[beam == 0] = np.inf
        beam = 1 / beam
        ...
        T_noise = (
            noise_ukarcmin
            * (np.pi / 180 / 60)
            * curvedsky.rand_map(shape, wcs, beam**2)
        )
...
class simple_sky_model_pol:
    def __init__(
        ...
        self.alm_cmb_E = curvedsky.rand_alm(
            cl_ee, lmax=lmax_sim
        )  # hp.synalm(cl_ee,lmax=3*nside-1,new=True)
        self.alm_cmb_B = curvedsky.rand_alm(
            cl_bb, lmax=lmax_sim
        )  # hp.synalm(cl_bb,lmax=3*nside-1,new=True)
        tmp_empty = enmap.empty(shape, wcs)
        self.Q_cmb, self.U_cmb = curvedsky.alm2map(
            np.array([self.alm_cmb_E, self.alm_cmb_B]), tmp_empty, spin=[2]
        )
        ...
        tmp_empty = enmap.empty(shape, wcs)
        self.Q_dust, self.U_dust = curvedsky.alm2map(
            np.array([self.alm_dust_E, self.alm_dust_B]), tmp_empty, spin=[2]
        )

        self.Q_dust *= galShape
        self.U_dust *= galShape

        self.alm_dust_E, self.alm_dust_B = curvedsky.map2alm(
            [self.Q_dust, self.U_dust], spin=[2], lmax=lmax_sim
        )

        tmp_empty = enmap.empty(shape, wcs)
        self.Q_sync, self.U_sync = curvedsky.alm2map(
            np.array([self.alm_sync_E, self.alm_sync_B]), tmp_empty, spin=[2]
        )

        self.Q_sync *= galShape
        self.U_sync *= galShape

        self.alm_sync_E, self.alm_sync_B = curvedsky.map2alm(
            [self.Q_sync, self.U_sync], spin=[2], lmax=lmax_sim
        )
...
class pysm_sky_model:
    ...
    def observe(self, freq_GHz, noise_ukarcmin=3.0, beam_fwhm_arcmin=8.0):
        ...
        sky = pysm3.Sky(
            nside=self.nside_pysm, preset_strings=["d1", "s1"], output_unit="K_CMB"
        )
        # Get the map at the desired frequency:
        I, Q_foreground, U_foreground = sky.get_emission(freq_GHz * u.GHz) * 1e6

        I, Q_foreground, U_foreground = reproject.enmap_from_healpix(
            [I, Q_foreground, U_foreground],
            shape,
            wcs,
            ncomp=3,
            unit=1,
